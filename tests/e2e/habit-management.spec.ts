...
test.describe('Habit Management', async () => {
  ...
  test('should add a new habit and display it in the habit list with correct summary', async ({
    ...
    await expect(
      page.getByRole('textbox', { name: 'Habit name' }),
    ).not.toBeVisible();
  ...
  test('should show an error notification when trying to add a duplicate habit name', async ({
    ...
    await expect(
      page.getByText('A habit with this name already exists'),
    ).toBeVisible();
  ...
  test('should edit a habit name and show a success notification', async ({
    ...
    await expect(
      page.getByText('Habit updated successfully'),
    ).not.toBeVisible();
  });
  ...
  test('should stop a habit and display the stopped status in the habit list', async ({
    ...
    await expect(
      page.getByText('Habit stopped successfully'),
    ).not.toBeVisible();
  ...
  test('should delete a habit and remove it from the habit list', async ({
    ...
    await expect(page.getByText('Habit deleted successfully')).toBeVisible();

    await page.waitForTimeout(3000);

    await expect(
      page.getByText('Habit deleted successfully'),
    ).not.toBeVisible();
